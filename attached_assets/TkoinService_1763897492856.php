...
class TkoinService
{
    ...
    public function __construct()
    {
        ...
        // Don't set base_uri - we'll use full URLs
    ...
    public function getUserBalance(User $user): ?array
    {
        try {
            ...
            Log::info('Fetched Tkoin balance', ['user_id' => $user->id, 'balance' => $response]);
            return $response;
        } catch (\Exception $e) {
            Log::error('Failed to fetch Tkoin balance', ['user_id' => $user->id, 'error' => $e->getMessage()]);
            return null;
        }
    }
    ...
    public function getUserTransactions(User $user, int $limit = 10): array
    {
        try {
            ...
            Log::error('Failed to fetch user transactions', ['user_id' => $user->id, 'error' => $e->getMessage()]);
            return [];
        }
    }
