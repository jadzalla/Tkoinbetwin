 ...
 * ✅ Account ID correctly extracted from API response (v6.5 fix)
...
class TkoinWallet {
  ...
  async fetchBalance() {
    try {
      ...
      if (data.success !== false) {
        // v6.5 FIX: Pass the FULL data object, not just data.balance
        // API returns: { balance: 1461.73, account_id: 1, ... }
        // We need the whole object to access account_id
  ...
  updateBalanceDisplay(data) {
    ...
    // v6.5 FIX: Handle both formats:
    // Format 1: { balance: 1461.73, account_id: 1 }
    // Format 2: { credits: 1461.73, account_id: 1 }
    const creditAmount = data.balance || data.credits || 0;
    ...
    // ============ ACCOUNT ID (v6.5 FIX) ============
    // Extract account_id from the API response
    const accountIdEl = this.getElement('balance-account', 'accountId');
    if (accountIdEl) {
      const accountId = data.account_id || data.user_id || data.id;
      console.log('[Tkoin] Account ID from API:', accountId, '(type:', typeof accountId, ')');
      
      if (accountId !== undefined && accountId !== null && accountId !== '' && accountId !== '---') {
        accountIdEl.textContent = 'Account ID: ' + accountId;
        console.log('[Tkoin] ✓ Updated account ID to:', accountId);
      } else {
        console.warn('[Tkoin] No valid account_id in API response');
      }
    }
  }
  ...
  updateHistoryDisplay(transactions) {
    // v6.5 FIX: Better element finding for transaction history
    let targetContainer = document.getElementById('transactionHistory');
    
    if (!targetContainer) {
      console.log('[Tkoin] transactionHistory not found - trying alternatives...');
      // Try alternative selectors
      targetContainer = document.getElementById('transaction-history') || 
                        document.querySelector('tbody[id*="transaction"]') ||
                        document.querySelector('.transaction-table tbody') ||
                        document.querySelector('#tkoin-wallet tbody') ||
                        document.querySelector('.tkoin-wallet-container tbody');
    }
    
    if (!targetContainer) {
      console.warn('[Tkoin] No transaction history container found - transactions:', transactions.length);
      return;
    }
    
    console.log('[Tkoin] Found transaction container:', targetContainer.id || targetContainer.className);
