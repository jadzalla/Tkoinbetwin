...
class TkoinWallet {
  ...
  async connectWallet() {
    ...
    try {
      console.log('[Tkoin] Requesting wallet connection via popup...');
      
      // Unblock auto-connect since user explicitly clicked
      this.autoConnectBlocked = false;
      
      // Force the connection popup even if already connected
      const response = await this.phantom.connect({ onlyIfTrusted: false });
      const pubKey = response.publicKey.toString();
      console.log('[Tkoin] Wallet connected via popup:', pubKey);
      
      this.handleWalletConnected(pubKey);
    } catch (error) {
      console.error('[Tkoin] Connection error:', error);
      this.autoConnectBlocked = true; // Re-block on error
      if (error.code === 4001) {
  ...
  updateBalanceDisplay(balance) {
    ...
    if (accountIdEl) {
      // Check multiple possible locations for account_id
      const accountId = balance.account_id || balance.user_id || balance.id;
      if (accountId) {
        accountIdEl.textContent = 'Account ID: ' + accountId;
        console.log('[Tkoin] ✓ Updated account ID to:', accountId);
      } else {
        console.warn('[Tkoin] No account_id found in balance response');
        console.log('[Tkoin] Balance object keys:', Object.keys(balance));
      }
    }
  }
