 ...
 * VERSION v7.0 - PHASE 1 ENHANCED (Filtering + Export + P2P Marketplace)
 * 
 * NEW in v7.0:
 * - Transaction filtering: type, status, date range
 * - Export transactions: CSV and JSON download
 * - Pagination support with offset-based navigation
 * - Filter state management and URL persistence
 * - P2P Marketplace integration (buy TKOIN from agents)
 * 
 * FIXES in v6.8:
 * - Disconnect persistence: Uses localStorage to prevent auto-reconnect after user disconnects
 * 
 * FIXES in v6.7:
...
class TkoinWallet {
  constructor() {
    ...
    // v7.0: Filter and pagination state
    this.filters = {
      type: 'all',
      status: 'all',
      date_from: '',
    ...
    console.log('[Tkoin] Initializing wallet manager v7.0 (PHASE 1 ENHANCED)');
  ...
  bindEvents() {
    ...
    const refreshBtn = document.getElementById('refreshBtn') || document.getElementById('btn-refresh');
    if (refreshBtn) {
    ...
    // v7.0: Filter event bindings
    this.bindFilterEvents();
    
    // v7.0: Export event bindings
    this.bindExportEvents();
    
    // v7.0: Pagination event bindings
    this.bindPaginationEvents();

    console.log('[Tkoin] Events bound successfully');
  }
  
  // v7.0: Bind filter control events
  bindFilterEvents() {
    ...
    const debouncedFetch = debounce(() => {
      this.pagination.offset = 0; // Reset to first page on filter change
      this.fetchHistory();
    }, 300);
    ...
    if (filterType) {
      filterType.addEventListener('change', (e) => {
        ...
        console.log('[Tkoin] Filter type changed:', this.filters.type);
        debouncedFetch();
      });
    }
    ...
    if (filterStatus) {
      filterStatus.addEventListener('change', (e) => {
        ...
        console.log('[Tkoin] Filter status changed:', this.filters.status);
        debouncedFetch();
      });
    }
    ...
    if (filterDateFrom) {
      filterDateFrom.addEventListener('change', (e) => {
        ...
        console.log('[Tkoin] Filter date from:', this.filters.date_from);
        debouncedFetch();
      });
    }
    ...
    if (filterDateTo) {
      filterDateTo.addEventListener('change', (e) => {
        ...
        console.log('[Tkoin] Filter date to:', this.filters.date_to);
        debouncedFetch();
      });
    }
    ...
    console.log('[Tkoin] Filter events bound');
  }
  
  // v7.0: Clear all filters
  clearFilters() {
    this.filters = { type: 'all', status: 'all', date_from: '', date_to: '' };
    ...
    console.log('[Tkoin] Filters cleared');
    this.fetchHistory();
  }
  
  // v7.0: Bind export button events
  bindExportEvents() {
    const exportCsvBtn = document.getElementById('btn-export-csv');
    const exportJsonBtn = document.getElementById('btn-export-json');
    
    if (exportCsvBtn) {
      exportCsvBtn.addEventListener('click', () => this.exportTransactions('csv'));
    }
    
    if (exportJsonBtn) {
      exportJsonBtn.addEventListener('click', () => this.exportTransactions('json'));
    }
    
    console.log('[Tkoin] Export events bound');
  }
  
  // v7.0: Export transactions
  async exportTransactions(format) {
    try {
      console.log('[Tkoin] Exporting transactions as', format);
      ...
      const filename = `tkoin-transactions-${new Date().toISOString().split('T')[0]}.${format}`;
      ...
      console.log('[Tkoin] Export completed:', filename);
      alert(`Downloaded: ${filename}`);
      
    } catch (error) {
      console.error('[Tkoin] Export error:', error);
      alert('Failed to export transactions: ' + error.message);
    }
  }
  
  // v7.0: Bind pagination events
  bindPaginationEvents() {
    ...
    console.log('[Tkoin] Pagination events bound');
  }
  ...
  checkPhantomWallet() {
    if (window.solana?.isPhantom) {
      ...
      // v6.8 FIX: Check if user previously disconnected
      const wasDisconnected = localStorage.getItem('tkoin_wallet_disconnected') === 'true';
      
      this.phantom.on('connect', () => {
        console.log('[Tkoin] Phantom connected event');
        
        // v6.8 FIX: If user previously disconnected, don't auto-connect
        if (wasDisconnected) {
          console.log('[Tkoin] User previously disconnected, ignoring auto-connect');
      ...
      // v6.8 FIX: If Phantom is already connected but user wanted to be disconnected, disconnect now
      if (this.phantom.isConnected && wasDisconnected) {
        console.log('[Tkoin] Disconnecting previously connected wallet (user preference)');
        this.phantom.disconnect();
      }
  ...
  async connectWallet() {
    ...
    try {
      ...
      // v6.8 FIX: Clear disconnect flag when user explicitly clicks Connect
      localStorage.removeItem('tkoin_wallet_disconnected');
      console.log('[Tkoin] Cleared disconnect preference');
  ...
  disconnectWallet() {
    // v6.8 FIX: Set disconnect flag to prevent auto-reconnect on page reload
    localStorage.setItem('tkoin_wallet_disconnected', 'true');
    console.log('[Tkoin] Saved disconnect preference');
  ...
  async fetchHistory() {
    try {
      console.log('[Tkoin] Fetching transaction history with filters:', this.filters);
      
      // v7.0: Build URL with filter and pagination parameters
      const params = new URLSearchParams();
      ...
      const url = `${this.apiBaseUrl}/history?${params.toString()}`;
      console.log('[Tkoin] Fetching from:', url);
      ...
      if (data.success !== false) {
        // v7.0: Update pagination state
        this.pagination.total = data.total || 0;
        this.pagination.has_more = data.has_more || false;
        
        this.updateHistoryDisplay(data.transactions || data.settlements || []);
        this.updatePaginationUI();
        this.updateFilterSummary(data.transactions?.length || 0, data.total || 0);
      }
    } catch (error) {
  ...
  // v7.0: Update pagination controls
  updatePaginationUI() {
    ...
    if (nextBtn) {
      nextBtn.disabled = !this.pagination.has_more;
    }
  ...
  // v7.0: Update filter summary
  updateFilterSummary(showing, total) {
    ...
    if (filterSummary && filterSummaryText) {
      if (hasFilters || total > this.pagination.limit) {
        filterSummary.style.display = 'block';
        filterSummaryText.textContent = `Showing ${showing} of ${total} transactions`;
      } else {
