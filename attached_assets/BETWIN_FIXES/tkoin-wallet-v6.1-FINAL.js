 ...
 * VERSION v6.1 - Token2022 Support + Corrected Element IDs
 * 
 * FIXES:
 ...
 * ✅ CORRECTED element IDs to match BetWin blade template
 */
...
class TkoinWallet {
  ...
  init() {
    console.log('[Tkoin] Initializing wallet manager v6.1 (Token2022 + Fixed IDs)...');
  ...
  setupEventListeners() {
    // Connect wallet button - CORRECTED ID: connect-wallet-btn
    const connectBtn = document.getElementById('connect-wallet-btn');
    if (connectBtn) {
      connectBtn.addEventListener('click', () => this.connectWallet());
      console.log('[Tkoin] Connect button listener attached');
    } else {
      console.warn('[Tkoin] Connect button not found (connect-wallet-btn)');
    }

    // Disconnect wallet button - CORRECTED ID: disconnect-wallet-btn
    ...
    // Refresh button - CORRECTED ID: btn-refresh
  ...
  updateWalletUI(connected) {
    // CORRECTED IDs: connect-wallet-btn, disconnect-wallet-btn
    const connectBtn = document.getElementById('connect-wallet-btn');
    const disconnectBtn = document.getElementById('disconnect-wallet-btn');
  ...
  showPhantomNotInstalled() {
    // CORRECTED ID: connect-wallet-btn
    const connectBtn = document.getElementById('connect-wallet-btn');
    if (connectBtn) {
  ...
  updateBalanceDisplay(balance) {
    // CORRECTED ID: balance-amount (was creditBalance)
    const balanceAmountEl = document.getElementById('balance-amount');
    if (balanceAmountEl) {
      const amount = parseFloat(balance.credits || balance || 0);
      balanceAmountEl.textContent = `${amount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} CREDIT`;
      console.log('[Tkoin] Updated balance-amount to:', amount);
    } else {
      console.warn('[Tkoin] balance-amount element not found');
    }
    
    // CORRECTED ID: balance-account (was accountId)
    const balanceAccountEl = document.getElementById('balance-account');
    if (balanceAccountEl && balance.account_id) {
      balanceAccountEl.textContent = 'Account ID: ' + balance.account_id;
      console.log('[Tkoin] Updated balance-account to:', balance.account_id);
    }
  }
  ...
  updateHistoryDisplay(transactions) {
    ...
    if (!container) {
      console.warn('[Tkoin] transactionHistory element not found');
      return;
    }
  ...
  async handleWithdraw() {
    ...
    const submitBtn = document.getElementById('withdrawalSubmit'); // CORRECTED ID
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('[Tkoin] DOM loaded, initializing TkoinWallet v6.1...');
