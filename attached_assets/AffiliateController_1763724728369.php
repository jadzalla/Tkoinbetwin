...
class AffiliateController extends Controller
{
    ...
    {
        $commission
            ->load('account', 'account.user', 'referralAccount', 'referralAccount.user', 'commissionable')
            ->makeVisibleAll();

        $commission->account->user->makeVisibleAll();
        $commission->referralAccount->user->makeVisibleAll();

        if ($commission->commissionable && method_exists($commission->commissionable, 'makeVisibleAll')) {
            $commission->commissionable->makeVisibleAll();
        }
    ...
    public function approve(AffiliateCommission $commission)
    {
        ...
        AccountTransactionRepository::createGeneric($commission->account, GenericAccountTransaction::TYPE_AFFILIATE_COMMISSION, $commission->amount);
